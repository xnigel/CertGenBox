...
DEFAULT_HASH = 'sha256'
# 调整：将默认路径设置为脚本所在目录下的 "certs" 子文件夹
...
class CertGenGUI(tk.Tk):
    ...
    def create_widgets(self):
        ...
        params_frame.pack(side=tk.TOP, fill=tk.X, pady=(0, 5))
        ...
        ttk.Label(params_frame, text='Signature algorithm:').grid(row=0, column=1, sticky='w', padx=(5,0))
        self.signalg_var = tk.StringVar(value=DEFAULT_HASH)
        ttk.Combobox(params_frame, textvariable=self.signalg_var, values=list(HASH_ALGORITHMS.keys()), width=12).grid(row=1, column=1, sticky='w', padx=(5,0))

        # Certificate Count
        ttk.Label(params_frame, text='Number of certs to generate (per type):').grid(row=2, column=0, sticky='w', pady=(5,0))
        ...
        # CA Fields
        ttk.Label(params_frame, text='CA Common Name:').grid(row=4, column=0, sticky='w', pady=(5,0))
        ...
        ttk.Label(params_frame, text='CA Email:').grid(row=4, column=1, sticky='w', padx=(5,0), pady=(5,0))
        self.ca_email_var = tk.StringVar(value="demo@example.com")
        ttk.Entry(params_frame, textvariable=self.ca_email_var, width=30).grid(row=5, column=1, sticky='w', padx=(5,0))
        ...
        # Output Path
        ttk.Label(button_frame, text='Output directory:').grid(row=0, column=0, sticky='w', pady=5)
        self.path_var = tk.StringVar(value=DEFAULT_PATH)
        ttk.Entry(button_frame, textvariable=self.path_var, width=50).grid(row=1, column=0, sticky='w', pady=5)
        ttk.Button(button_frame, text='Browse...', command=self.browse_path).grid(row=1, column=1, sticky='w', pady=5)

        # 调整：将 "Run Generation" 和 "Open Output Folder" 放在同一行
        action_button_frame = ttk.Frame(frm)
        action_button_frame.pack(side=tk.TOP, fill=tk.X, pady=(0, 5))
        ...
        # Log
        ttk.Label(frm, text='Log:').pack(anchor='w', pady=(5,0))
