...
DEFAULT_CERT_COUNT = 2 # 每种类型的证书数量 默认值可以设定为5
...
def generate_all(output_path, log_cb=print, keysize=2048, sign_alg='sha256', ca_cn="TESTCA", ca_email="nigel.zhai@ul.com", cert_count=DEFAULT_CERT_COUNT):
...
class CertGenGUI(tk.Tk):
    def __init__(self):
        ...
        # 调整: 添加窗口图标
        try:
            # 移除ICON_PNG_BASE64字符串中的所有空格和换行符
            cleaned_icon_data = ICON_PNG_BASE64.strip().replace(" ", "").replace("\n", "")
            icon_photo = tk.PhotoImage(data=cleaned_icon_data)
            self.iconphoto(False, icon_photo)
        except (binascii.Error, tk.TclError) as e:
    ...
    def create_widgets(self):
        ...
        ttk.Label(params_frame, text='Signature algo:').grid(row=0, column=1, sticky='w', padx=(10,0))
        ...
        # Certificate Count
        ttk.Label(params_frame, text='No. of Certs per type:').grid(row=0, column=2, sticky='w', pady=(8,0))
        self.cert_count_var = tk.IntVar(value=DEFAULT_CERT_COUNT)
        ttk.Entry(params_frame, textvariable=self.cert_count_var, width=10).grid(row=1, column=2, sticky='w')

        # CA Fields
        ttk.Label(params_frame, text='CA Common Name:').grid(row=2, column=0, sticky='w', pady=(8,0))
        self.ca_cn_var = tk.StringVar(value="TESTCA")
        ttk.Entry(params_frame, textvariable=self.ca_cn_var, width=10).grid(row=3, column=0, sticky='w')

        ttk.Label(params_frame, text='CA Email:').grid(row=2, column=1, sticky='w', padx=(10,0), pady=(8,0))
        self.ca_email_var = tk.StringVar(value="nigel.zhai@ul.com")
        ttk.Entry(params_frame, textvariable=self.ca_email_var, width=20).grid(row=3, column=1, sticky='w', padx=(10,0))
        ...
        # Output Path
        ttk.Label(button_frame, text='Output directory:').pack(side=tk.LEFT, padx=5)
        self.path_var = tk.StringVar(value=DEFAULT_PATH)
        ttk.Entry(button_frame, textvariable=self.path_var, width=45).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text='Browse...', command=self.browse_path).pack(side=tk.LEFT, padx=5)
        ...
        # 使用 ttk.Style 来设置按钮背景颜色
        style = ttk.Style()
        style.configure("Run.TButton", background="#D1FFBD", font=('Helvetica', 9, 'bold'))
        ttk.Button(action_button_frame, text='Generate!', command=self.on_run, style="Run.TButton").pack(side=tk.RIGHT, padx=5)
        ttk.Button(action_button_frame, text='Open folder', command=self.open_output).pack(side=tk.RIGHT, padx=5)

        # 使用 tk.Button 代替 ttk.Button 来实现背景颜色完全填充
        # tk.Button(action_button_frame, text='Run Generation', command=self.on_run, bg="#D1FFBD").pack(side=tk.RIGHT, padx=5)
        # tk.Button(action_button_frame, text='Browse Certs', command=self.open_output).pack(side=tk.RIGHT, padx=5)
